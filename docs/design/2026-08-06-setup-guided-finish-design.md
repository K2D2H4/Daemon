...
`Wizard._finish`에 **"Keep it running?" 마무리 섹션**을 추가하고, 지금의 수동
"Next:" 세 줄을 이 섹션이 대체한다.

번호 없는 섹션으로 둔다(`STEPS`는 5 그대로, "Review - .env" 헤딩과 같은 방식).
번호를 6으로 올리면 비대화형 실행에서 헤딩은 5개만 찍히는데 분모만 `/6`이 되어
오해를 준다(페어링이 조건부인데 5로 세는 것과 같은 잠재 문제). 마무리 섹션으로
두면 numbering 테스트도 건드리지 않고, 스킵/거절한 사용자에게 헛된 카운트도 안
보인다.
