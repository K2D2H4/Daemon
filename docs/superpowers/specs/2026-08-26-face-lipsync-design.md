...
| 전송 | **서버가 crop box 프레임, SSE** (2026-08-26 수정) | 렌더러가 발화 중간에 `failed`를 래치할 수 있고, 페이지는 그것을 알아채고 클립으로 돌아가야 한다. 그러려면 **프레임별 도착 시각과 스트림 종료 신호**가 필요한데 `multipart/x-mixed-replace`를 `<img>`에 물리면 둘 다 이식성 있게 얻을 수 없다 (Chrome은 스트림 전체에 `load`를 한 번 쏘고, 정상 종료는 아무 이벤트도 내지 않는다). `EventSource`는 `onmessage` + `readyState === CLOSED`로 둘 다 공짜로 주고, `/face/stream`과 `face.html`이 이미 쓰는 관용구다. 대가는 base64 33%: crop 55KB → 73KB, 10.8 → 14.4 Mbit/s — 원래 승인된 **완성 프레임** MJPEG의 35.5 Mbit/s보다 여전히 2.5배 적다. crop으로 바뀐 이유는 §3의 렌더 예산이 이미 crop 인코딩(0.46ms)을 전제하고 있었다는 것이고, 실측은 `daemon/face_routes.py:frames`에 있다 |
...
```
재생 경과 → 구동 프레임 인덱스
PCM 링에서 200ms 윈도 (과거 120ms / 미래 최대 80ms, 61.67~80.0 순환)
...
## 4-1. 전처리는 Apple Vision을 쓴다 — 런타임에는 torch가 없다

**2026-08-26 수정.** 이 절의 원래 제목은 "런타임도 빌드도 torch가 없다"였고, 그건 도달할 수
없는 목표였다. 전처리에는 68점 랜드마크 말고도 **블렌드 마스크**가 필요하고, 그것은
`musetalk.utils.face_parsing.FaceParsing` — BiSeNet + torchvision transform — 이며 이 저장소에
대체물이 없다. 그래서 결정을 바꾼다:

- **전처리는 별개의 오프라인 프로세스이고 거기서는 torch를 쓴다.** `evals/face_lipsync_prepare.py`가
  그것이고, 자기 인터프리터에서 손으로 돌린다(모듈 docstring에 설치 방법이 있다). CI는 절대
  돌리지 않고, 데몬은 절대 import하지 않는다.
- **§7의 "두 프레임워크를 한 프로세스에서 함께 돌리지 않는다"는 런타임 규칙이고 그대로다.**
  렌더 경로는 여전히 전부 MLX다 — `daemon/face_lipsync/*`는 `mlx`, `numpy`, `cv2`만 import하고
  torch를 import하는 코드는 `daemon/` 아래 한 줄도 없다. 드리프트 +41.2%를 만든 것은 *한
  프로세스 안의* MLX+MPS 공존이었고, 별 프로세스로 갈라놓으면 그 실패는 존재하지 않는다.
- **VAE는 장애물이 아니었다.** 참조 잠재는 `sd-vae-ft-mse`의 인코더로 만들고(§4가 이름을 빠뜨린
  모델 — `evals/face_lipsync_prepare.py` 모듈 docstring에 측정과 함께 있다), TAESD 인코더로 바꾸면
  입술 채도 -5.7%, 콘트라스트 -8.6%로 눈에 보이게 나빠진다. MLX로 포팅해도 `diffusers`와 335MB만
  빠지고 torch는 그대로 남는다.
- **열린 항목: BiSeNet을 Vision 파생 턱 마스크로 대체한다.** 전처리에서 torch를 정말로 없애려면
  이것이 남은 하나다. Vision은 이미 `faceContour`와 `outerLips`를 주므로 턱 아래 영역의 다각형
  마스크를 그리고 블러할 수 있다. 화질에 영향을 주는 변경이므로 §4-1의 검증과 같은 방식으로
  — 렌더된 입을 보고 — 판정해야 한다. 지금은 하지 않는다.

원래 이 절이 옳게 말한 부분은 그대로다: 주인이 자기 클립을 만들 수 있어야 하므로 전처리는
**돌아가는 스텝**이어야 하고, 캐시만 배포하고 끝낼 수 없다. 랜드마크에 대해서는 결정도 그대로다.
...
**`GET /face/frames`** — **SSE, crop box JPEG 하나가 이벤트 하나** (2026-08-26 수정;
원래 `multipart/x-mixed-replace` + 완성 프레임이었다. 이유는 §승인된 결정의 전송 행에
있다). 발화 중에만 흐르고 그 외에는 열려만 있다. `idle2` 실측 729x763 crop에서
71KB/프레임, 20fps에서 11.4 Mbit/s — 루프백이다.

기존 **`/face/stream` SSE는 손대지 않는다.** activity·level을 계속 실어 보낸다.

`face.html`은 매니페스트가 립싱크 가용과 **crop box 좌표**를 알리고, activity가
`speaking`일 때 speaking 클립 대신 구동 클립 위에 crop을 얹는다. 좌표가 프레임이 아니라
매니페스트에 타는 이유: `<img>`는 SSE 이벤트의 헤더를 읽을 수 없고, box는 클립 전체에 대해
하나로 고정돼 있다(프레임별 crop box의 합집합). §3.2의 "들어오는 요소만 페이드" 규칙
그대로다.
