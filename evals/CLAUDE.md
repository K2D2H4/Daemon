...
| `voice_room_spike.py` | whether the daemon answers **the person** and ignores **the room** - the question a fake `AudioIO` cannot ask, and the one the owner asked on 2026-09-02 when the daemon began answering its own leaked voice (5 of 27 user turns were the suffix of the reply just played). Drives the product with a real microphone: a person played through a chosen speaker, the answer through the system default, `VoiceProcessingAudio` in between, and every turn the server hears scored as answered / spurious. **Its own stand-in fades:** the same TTS read 72% speech frames at the microphone early in a run and 0-2% an hour later, because macOS voice processing converges on this machine's own speakers - so it measures its input first and refuses to report anything when that has gone quiet |
