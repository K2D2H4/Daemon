...
| `proactive_verbatim_spike.py` | whether the live voice model will say a given sentence **exactly** - the question PR #115 answered by argument and got wrong. Two cells, 8 live sessions each: a plain instruction is exact **0/8** (it says the line, then adds a question of its own), `conversation.SPEAK_VERBATIM` is **8/8**. Scores the constant *as imported*, so it measures the string that ships; re-run it after any edit to that string, or on another model |
