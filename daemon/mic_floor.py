...
Note what does *not* separate them: whether the microphone is held. On
`_speak_unprompted` - the path that takes a line and speaks it - nothing is holding
it when the answer comes back: `listen()`'s `finally` closes the capture stream,
which exits `record()`'s hold, before `_wake_round` closes the gate, and the voice
session that path opens (PR #126) has ended by the time it answers `not-spoken`. An
earlier version of this paragraph said `not-spoken` meant the wake loop still had
the device (PR #115 review). On that path it does not, and the real reasons are in
`ProactiveDelivery._say`.

Scoped to that path deliberately, because one `not-spoken` *does* come back with the
device held and it is the one nobody should generalise from: `_wake_round`'s
wedged-microphone branch answers `False` without speaking at all, precisely because
`close_gate()` reported that the capture stream never came back. Nothing downstream
is hurt - `_say` does not reach for the speaker on `not-spoken` anyway, and on
`no-listener` it relies on exactly that wedged gate holding `mic_hold` for its 45 s
watchdog so `LocalSpeaker.say` refuses (`daemon/proactivity/delivery.py`).

`spoke` is the answer that now arrives from inside a live session that *is* holding
the device: PR #126 answers it on the first chunk of the session's audio
(`VoiceConversation._on_audio`) rather than when the conversation ends, because the
conversation has no total cap and `REPLY_CEILING_SECONDS` does. That costs nothing
here, since `spoke` is the answer that sends the caller nowhere near the speaker.
