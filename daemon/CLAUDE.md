...
| `face_routes.py` | the face's HTTP surface: `/face` (`daemon/static/face.html`), `/face/stream` (SSE off the bus), `/face/manifest`, `/face/clips/{name}` reading `<data_dir>/face/*.mp4` - an allowlisted name, not a path. Read-only and carries no conversation, same loopback reasoning as `admin/routes.py` |
