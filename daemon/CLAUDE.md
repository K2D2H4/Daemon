...
| `face_lipsync/` | the second mouth, behind `DAEMON_FACE_LIPSYNC_ENABLED` (default off): `face_lipsync/engine.py` (the only place a model runs - MuseTalk's UNet, whisper's encoder and TAESD, all MLX, verified against the PyTorch pipeline at cosine 0.999707) · `face_lipsync/render.py` (`Renderer`, `FrameClock`, `restore_detail`) · `face_lipsync/ring.py` (`PcmRing`, `Slot`) · `face_lipsync/audio.py` (whisper windowing) · `face_lipsync/loader.py` · `face_lipsync/taesd.py` · `face_lipsync/vendor/mlx_sd/`. **Imports nothing from `daemon/`**, so `app.py:_build_lipsync` is the only file that names it; the cache it reads is built by hand with `python3 -m evals.face_lipsync_prepare` |
| `face_routes.py` | the face's HTTP surface: `/face` (`daemon/static/face.html`), `/face/stream` (SSE off the bus), `/face/manifest`, `/face/transitions` (the pose-match table, 404 until it is built), `/face/frames` (the lip-synced mouth, 503 until `_build_lipsync` has assembled one), `/face/clips/{name}` reading `<data_dir>/face/*.mp4` - an allowlisted name, not a path. Read-only and carries no conversation, same loopback reasoning as `admin/routes.py` |
...
| `voice/` | `voice/base.py` (frozen) · `voice/gemini_live.py` · `voice/vertex.py` (the second endpoint for the same protocol: regional URI, bearer-token provider, project-qualified model path — [ADR 0020](../docs/adr/0020-two-endpoints-serve-gemini-live.md)) · `voice/audio.py` (PortAudio) · `voice/apple_audio.py` (macOS echo cancellation) · `voice/conversation.py` · `voice/vad.py` · `voice/speech_gate.py` (only a person talking reaches the model; the room and the echo canceller's leaks go out as digital silence - fitted to what the owner's microphone actually carries, [MEASURED](MEASURED.md)) · `voice/apple_speech.py` · `voice/wake.py` · `voice/mic_access.py` (mic TCC request + status, Apple-guarded) |
...
| `proactivity/` | `proactivity/base.py` (frozen) · `candidates` · `gate` · `presence` · `judge` · `delivery` · `speaker` · `tick` · `topics` (type F's one web search, [ADR 0015](../docs/adr/0015-code-may-search-where-the-model-may-not.md)) · `agenda` (type G's one calendar read, and the only one that happens *before* the gate — [ADR 0021](../docs/adr/0021-the-calendar-read-happens-before-the-gate.md)) |
