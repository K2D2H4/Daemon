...
| `voice/` | `voice/base.py` (frozen) · `voice/gemini_live.py` · `voice/vertex.py` (the second endpoint for the same protocol: regional URI, bearer-token provider, project-qualified model path — [ADR 0020](../docs/adr/0020-two-endpoints-serve-gemini-live.md)) · `voice/audio.py` (PortAudio) · `voice/apple_audio.py` (macOS echo cancellation) · `voice/conversation.py` · `voice/vad.py` · `voice/apple_speech.py` · `voice/wake.py` · `voice/mic_access.py` (mic TCC request + status, Apple-guarded) |
